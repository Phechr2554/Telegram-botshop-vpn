...
| `DB_PATH` | แนะนำ | Path SQLite เช่น `/data/bot.db` บน Railway; ถ้าไม่ตั้งค่าจะใช้ `data/bot.db` ข้าง `database.py` |
...
### Bot-Hosting / Pterodactyl-style hosting

ถ้า deploy บน Bot-Hosting แล้วบอทขึ้น `PermissionError` ที่ `os.makedirs(.../data...)`
แปลว่า path `/data` ไม่มีสิทธิ์เขียนใน container นั้น เวอร์ชันนี้มี fallback ให้อัตโนมัติ
ไปยัง `data/bot.db` ภายในโฟลเดอร์โปรเจกต์ จึงไม่ควร crash ตั้งแต่ `db.init_db()` อีกต่อไป

ถ้าโฮสต์มี persistent storage ให้ตั้ง `DB_PATH` เป็นโฟลเดอร์ที่โฮสต์อนุญาตให้เขียน
เช่น `/home/container/data/bot.db` (ชื่อ path อาจต่างกันตามโฮสต์)

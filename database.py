...
# ---------------------------------------------------------------------------
# Railway can use a persistent volume mounted at /data, while Bot-Hosting and
# other container hosts commonly only allow writes under /home/container.
# Respect an explicit DB_PATH, but automatically fall back to a writable
# application-local directory when the configured path cannot be created.
_APP_DIR = os.path.dirname(os.path.abspath(__file__))
_FALLBACK_DB_PATH = os.path.join(_APP_DIR, "data", "bot.db")
_CONFIGURED_DB_PATH = os.getenv("DB_PATH", "").strip()

# Keep the traditional Railway default when DB_PATH is not specified there,
# but do not let an unwritable /data mount crash the whole bot on startup.
DB_PATH = _CONFIGURED_DB_PATH or _FALLBACK_DB_PATH
...
def _is_writable_db_parent(path: str) -> bool:
    """Ensure the DB parent exists and is writable."""
    parent = os.path.dirname(os.path.abspath(path))
    try:
        os.makedirs(parent, exist_ok=True)
        # Check directory write/execute access without creating a permanent file.
...
def _get_conn() -> sqlite3.Connection:
    ...
    # First try the configured path (e.g. /data/bot.db on Railway).
    if _is_writable_db_parent(DB_PATH):
        try:
            return _connect(DB_PATH)
        except (OSError, sqlite3.OperationalError, sqlite3.DatabaseError):
            pass

    # Fallback for hosts where /data is read-only or forbidden.
    if os.path.abspath(DB_PATH) != os.path.abspath(_FALLBACK_DB_PATH):
        if _is_writable_db_parent(_FALLBACK_DB_PATH):
            try:
                logger_path = f" (configured DB_PATH={DB_PATH!r} ใช้งานไม่ได้)"
                print(
                    f"⚠️ SQLite: ใช้ path สำรอง {_FALLBACK_DB_PATH!r}{logger_path}",
                    flush=True,
                )
    ...
    raise RuntimeError(
        ...
        f"DB_PATH={DB_PATH!r}. "
        "ตรวจสอบสิทธิ์เขียน directory หรือกำหนด DB_PATH เป็น path ที่เขียนได้"
    )
